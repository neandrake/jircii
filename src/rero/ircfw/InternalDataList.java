...
public class InternalDataList {
	...
	protected HashMap wasOn = new HashMap(); /*
											 * key=String value=Set hashmap containing nick->channel mappings for all users
											 * who quit. Upon its first access though the value is removed.
											 */
	...
	protected int maxModes = 3; // Maximum # of modes per line, gotten from MODES variable from 005 server message; Default
								// on EFnet is 4, but set default to 3 to be safe as this is standard.
	...
	public boolean isChanGroupMode(String group, char mode) {
		if (!chanModes.containsKey(group)) {
			return false;
		}
