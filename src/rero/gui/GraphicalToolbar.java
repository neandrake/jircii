...
public class GraphicalToolbar extends JToolBar {
	...
	/**
	 * this method is cheating and I should feel very guilty for doing my toolbar code like this, I'll probably pay for it
	 * later
	 */
	...
	public GraphicalToolbar() {
		...
		if (ClientState.getClientState().isOption("ui.sdi", ClientDefaults.ui_sdi)) {
			...
			actions = new ToolAction[] { new ConnectAction(), null, new OptionsAction(), new ScriptAction(), // popup menus
																												// aka script
																												// manager
			null, new ListAction(), // list action
			null, new SendAction(), // dcc send
			new ChatAction(), // dcc chat
			new DCCAction(), // dcc options
			null, new NotifyAction(), // notify list options
			new NotifyAction2(), // notify list options
			null, new HelpAction(), new AboutAction(), new EvilAction() };
		} else {
			...
			null, new NotifyAction(), // notify list options
			new NotifyAction2(), // notify list options
			null, new CascadeAction(), new TileAction(), null, new HelpAction(), new AboutAction(), new EvilAction() };
		}
