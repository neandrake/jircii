...
public class UIScriptBridge {
	...
	protected WindowClickListener windowDoubleClick; // handles the "on dclick" event for text being (double) clicked on in a
														// listbox...
	protected WindowClickListener windowSpecialClick; // handles the "on sclick" event for text being (double) clicked on in
														// a special window
	...
	public void windowCreated(final StatusWindow window) {
		...
		windowState.onOpen(new ClientWindowEvent(window.getWindow())); // fake out the scripting saying
