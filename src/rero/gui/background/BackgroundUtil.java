...
public class BackgroundUtil {
	...
	/**
	 * draws the image to fill the specified component, hardware scaling is pretty damned fast but we still cache the scaled
	 * image anyways just to be safe
	 */
	...
	public static void drawImageStretched(Component source, Graphics g, BackgroundProperties properties) {
		...
		if (temp == null || temp.getWidth(null) != (int) compare.getWidth() || temp.getHeight(null) != (int) compare.getHeight()) {
			...
			float x_diff = ((float) compare.getWidth() / imageData.getWidth(null));
			float y_diff = ((float) compare.getHeight() / imageData.getHeight(null));
