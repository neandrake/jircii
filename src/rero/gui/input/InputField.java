...
public class InputField extends JTextField implements KeyListener, ActionListener, MouseListener, ClientStateListener {
	...
	public void cleanup() {
		/*
		 * apparently this unholy triad of stuff is responsible for Java holding on to each session...
		 */
	...
	public InputField() {
		...
		defaultBorder = BorderFactory.createEmptyBorder(1, TextSource.UNIVERSAL_TWEAK, 1, 1); // a 1 pixel empty border all
																								// around;
		setBorder(defaultBorder);
		/*
		 * setBackground(null); // suggested by Sun as a fix to a background being painted problem // in the GTK+ look and
		 * feel, unfortunately it doesn't work... maybe it will when 1.5 comes out
		 */
	...
	public void keyPressed(KeyEvent e) {
		...
		if (e.getKeyCode() == KeyEvent.VK_UP) {
			...
			if (commandIterator.hasPrevious()) {
				...
				// Set text from history
					...
					setText(previous);
					// System.out.println("Up key; printed \"previous\" text; caretPos = " + getCaretPosition() +
					// ", length = " + previous.length() + ", string = \"" + previous + "\"; text length = " +
					// getText().length());
				}
		...
		if (e.getKeyCode() == KeyEvent.VK_DOWN) {
			...
			if (commandIterator.hasNext()) {
				...
				// command line
				if (commandIterator.hasNext() && commandIterator.nextIndex() + 1 == commandHistory.size() && this.previousKeyUpArrow) {
	...
	private void addToHistory(String text) {
		...
		if (this.commandHistory.size() == (maxCommands + 1) && (maxCommands > 0)) {
