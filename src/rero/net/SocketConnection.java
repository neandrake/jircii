...
public class SocketConnection implements Runnable, ClientStateListener {
	...
	public void disconnect() {
		if (aSocket != null && aSocket.isConnected()) {
			try {
				if (!aSocket.isOutputShutdown()) {
					aSocket.shutdownOutput();
				}
				if (!aSocket.isInputShutdown()) {
	...
	public void connect(String host, int port, long _delay, String password, boolean secure) {
		...
		// System.out.println(getSocketInformation().hostname + ", " + getSocketInformation().port + ", " +
		// getSocketInformation().isSecure + ", " + getSocketInformation().password);
	...
	private void shutdownSocket() {
		try {
			if (aSocketInput != null) {
				aSocketInput.close();
			}
			if (aSocketOutput != null) {
				aSocketOutput.close();
			}
			if (aSocket != null) {
				aSocket.close();
