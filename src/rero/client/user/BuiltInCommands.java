...
public class BuiltInCommands extends Feature implements ClientCommand {
	...
	public void runAlias(String command, String parms) {
		...
		ArrayList parametersArray = parmsToArrayList(parms, true); // No empty parameters included (no whitespace-only
																	// parameters)
		ArrayList parametersArrayFull = parmsToArrayList(parms, false); // Includes empty parameters (parameters can have
																		// whitespace)
	...
	// Create homogenous channel MODE line(s) (that is, one mode type e.g. +o), and return it as an array with one MODE line
	// per array element.
	// Make sure the 'mode' passed in is always lower-case UNLESS the mode is, per some odd ircd, upper-case. Targets should
	...
	public ArrayList createHomogenousChannelModesList(String channel, String modifier, String mode, ArrayList targets) {
		...
		// TODO: Check for valid channel prefix, based on the information provided by the IRC server (e.g. some servers have
		// more than '&' and '#').
		...
		ArrayList modeLines = new ArrayList(); // Defaults to 10 initially, per Java documentation. This probably won't ever
												// be exceeded, but it'll grow if it needs to!
	...
	public void connectToServer(String parms) {
		...
		/*
		 * Convoluted ass code ... while (!stack.isEmpty()) { temp = stack.pop(); if (loopCnt < 2) { if
		 * (temp.toLowerCase().equals("-ssl") || temp.toLowerCase().equals("-s")) { secure = true; } else if
		 * (temp.toLowerCase().equals("-pass") || temp.toLowerCase().equals("-p")) { if (stack.isEmpty()) { // Needed a
		 * password, but didn't get one.
		 * getCapabilities().getUserInterface().printStatus("/server error: no password specified; no server specified..");
		 * return; } else { password = stack.pop(); } } else { if (loopCnt == 0) host = temp; else if (loopCnt == 1) { if
		 * (secure == true || password != null) { host = temp; } else { port = Integer.parseInt(temp); } } } } else { if
		 * (loopCnt == 2) { if (secure == true && password != null) { host = temp; } else if (secure == true && password ==
		 * null) { try { port = Integer.parseInt(temp); } catch (NumberFormatException ex) {
		 * getCapabilities().getUserInterface().printStatus("/server error: invalid port number specified."); return; } } }
		 * else if (loopCnt == 3) { if (secure == true && password != null && host != null) { try { port =
		 * Integer.parseInt(temp); } catch (NumberFormatException ex) {
		 * getCapabilities().getUserInterface().printStatus("/server error: invalid port number specified."); return; } } } }
		 * 
		 * loopCnt++; }
		 * 
		 * if (host == null) {
		 * getCapabilities().getUserInterface().printStatus("/server error: must specify a server name."); return; }
		 */
