...
public class ScriptManager extends Feature implements ClientStateListener, RuntimeWarningWatcher {
	...
	public void hashScripts() {
		// TODO: When we load, traverse all the loaded scripts and, if the path is not absolute, change it to absolute and
		// save it. We must enforce absolute paths!
		...
		// Traverse through the loaded scripts and correct the script paths to absolute paths, as that's the format in which
		// we've stored them.
		...
		while (i.hasNext()) {
			...
			if (!loader.isLoaded(tt)) {
				internalScriptLoad(tt); // this check is overly redundant, however if a script loads a script while this
										// operation is
	...
	// Checks whether a script is in the script.files property list. It determines via getAbsolutePath() on each iteration of
	// script.files, and of the passed argument filename.
	...
	private void setDebugReal(String filename, String debuglevel) {
		...
		if (script == null) {
			// getCapabilities().getUserInterface().printStatus("Could not find loaded script " + filename +
			// " to set debug level.");
			return;
		}
	...
	private boolean internalScriptLoad(String scriptFile) {
		...
		if (!fScript.exists()) {
			...
			// System.err.println("internalScriptLoad(): Cannot load script, check permissions and file type for script " +
			// scriptFile);
			return false;
		}
		...
		try {
			...
			/*
			 * else { loader.loadScript(scriptFile, environment); }
			 */
