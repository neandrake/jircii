...
/**
 * Responsible for the following: - miscellaneous features i.e. responding to server PING's etc - send altnick IF we're not
 * connected yet and get a reply of nick in use.
 */
		...
		if (event.equals("376")) // end of MOTD command.
		{
			if (restoreInformation != null) {
				...
				while (i.hasNext()) {
					...
					getCapabilities().sendln("JOIN " + temp.getName() + " :" + temp.getKey()); // TODO: Investigate -- is
																								// this right? With a colon
																								// before the key? Because
																								// JOIN in
																								// BuiltInCommands.java
																								// doesn't use the colon.
				}
		...
		public int fireChatEvent(HashMap desc) {
			...
			if (event.equals("433")) {
				if (rero.test.QuickConnect.IsQuickConnect() && rero.test.QuickConnect.GetInformation().getURL().getUserInfo() != null) {
					// set a new alternate alt nick iff a user is quick connecting and a default nickname has been
					// specified...
